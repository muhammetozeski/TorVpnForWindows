...
/// <summary>
/// Last-resort copy of the built-in bridge lines, taken from
/// https://bridges.torproject.org/moat/circumvention/builtin on 2026-09-05.
///
/// <see cref="BridgeProvider"/> fetches the current list from that endpoint and caches it; this
/// copy is only used when the endpoint cannot be reached and no cache exists yet. It is frozen at
/// the moment this version was built, while the real list changes as bridges are retired, so it
/// should be treated as a starting point rather than a current answer.
///
/// These bridges are public, so a censor that cares can block them. Bridges requested individually
/// from https://bridges.torproject.org can be pasted into the custom list instead.
/// </summary>
public static class BuiltInBridges
{
    public static IReadOnlyList<string> Obfs4 { get; } =
    [
    ...
    public static IReadOnlyList<string> Snowflake { get; } =
    [
    ...
    public static IReadOnlyList<string> Meek { get; } =
    [
        "meek_lite 192.0.2.20:80 url=https://1603026938.rsc.cdn77.org front=www.phpmyadmin.net utls=HelloRandomizedALPN"
    ];

    public static IReadOnlyList<string> For(BridgeMode mode) => mode switch
    {
